...
这个仓库当前按“`GitHub Actions + Telegram` 每日热点推送”做了最小化配置，目标是先稳定跑起来。

- 运行方式：`GitHub Actions` 每 2 小时运行一次，用来持续采集当天热点数据
- 推送方式：`Telegram`
- 推送频率：北京时间 `20:00-22:00` 窗口内只推送 `1` 次
- 推送内容：`daily` 当日汇总
- 当前简化项：已关闭 `AI 分析`、`AI 翻译`、`RSS`，避免额外 API 成本和噪音

首次启用前，请在仓库 `Settings -> Secrets and variables -> Actions` 中添加这两个仓库级 Secret：

- `TELEGRAM_BOT_TOKEN`
- `TELEGRAM_CHAT_ID`

然后到仓库 `Actions` 页面启用工作流；如果 GitHub 默认禁用了 Actions，需要先点击启用。
...
### 2️⃣ 第二步：设置 GitHub Secrets
   ...
   **设置步骤：**
   ...
   1. **查看生成的 URL**：
      ...
      - 格式如：`https://hooks.slack.com/services/<team>/<channel>/<token>`
